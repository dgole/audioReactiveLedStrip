...
class Runner:
    def __init__(self, n, speed, color, startLoc):
        ...
        if self.speed > 0:
            self.outArray[self.locInt-self.n:self.locInt] = 1.0
        else:
            self.outArray[self.locInt:self.locInt+self.n] = 1.0
    def update(self):
    ...
    def getFullOutArray(self):
        if self.color=='r':
            return np.flatten([self.outArray, self.outZeros, self.outZeros])
        elif self.color=='g':
            return np.flatten([self.outZeros, self.outArray, self.outZeros])
        elif self.color=='b':
            return np.flatten([self.outZeros, self.outZeros, self.outArray])
