...
r_filt = dsp.ExpFilter(np.tile(0.01, config.N_PIXELS), alpha_decay=0.05, alpha_rise=0.1)
g_filt = dsp.ExpFilter(np.tile(0.01, config.N_PIXELS), alpha_decay=0.05, alpha_rise=0.1)
b_filt = dsp.ExpFilter(np.tile(0.01, config.N_PIXELS), alpha_decay=0.05, alpha_rise=0.1)
common_mode = dsp.ExpFilter(np.tile(0.01, config.N_PIXELS), alpha_decay=0.99, alpha_rise=0.01)
p_filt = dsp.ExpFilter(np.tile(1, (3, config.N_PIXELS)), alpha_decay=0.1, alpha_rise=0.99)
p = np.tile(1.0, (3, config.N_PIXELS))
gain = dsp.ExpFilter(np.tile(0.01, config.N_FFT_BINS), alpha_decay=0.1, alpha_rise=0.99)
...
def visualize_spectrum(y):
    ...
    bassMean = np.mean(r[:5])
    #r = np.ones_like(b) * bassMean * 5.0

    #highMean = np.mean(b[-20:])
    #highPixels = int(highMean*100.0)
    #b = np.zeros_like(b)
    #b[-highPixels:] = 0.5	
