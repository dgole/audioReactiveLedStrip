...
from numpy import abs, append, arange, insert, linspace, log10, round, zeros, log, concatenate, zeros_like
...
def hertz_to_mel(freq):
    ...
    """
    return round(12.0*(log(0.0323963*freq)/0.693147)+12.0)
...
def melfrequencies_mel_filterbank(num_bands, freq_min, freq_max, num_fft_bands):
    ...
    for i in range(len(center_frequencies_mel)):
		...
		elif center_frequencies_mel[i] < 37+12+12:
			lower_edges_mel[i] = cent - 0.75  
			upper_edges_mel[i] = cent + 0.75
		elif center_frequencies_mel[i] < 37+12+12+12:
			lower_edges_mel[i] = cent - 0.5  
			upper_edges_mel[i] = cent + 0.5
		else: 
			lower_edges_mel[i] = cent - 0.25  
			upper_edges_mel[i] = cent + 0.25
...
def compute_melmat(num_mel_bands=12, freq_min=64, freq_max=8000,
    ...
    freqs = linspace(0.0, freq_max*4.0, num_fft_bands)
    ...
    print(freqs)
    print(melmat.shape)
    print(melmat[10,0:200])
    print(melmat[11,0:200])
    print(melmat[12,0:200])
    print(melmat[13,0:200])
